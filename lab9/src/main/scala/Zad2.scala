...
def zad2: Unit = {
 ...
 val województwa = List(
  ...
  val wynikiPerWojewództwo = województwa.map { w =>
    val wynikiWoj = wyniki.filter(wynik => wynik.ID >= w.min && wynik.ID < w.min + 20000)
    val sumKO = wynikiWoj.map(_.KOALICJA_OBYWATELSKA).sum
    val sumPiS = wynikiWoj.map(_.PIS).sum
    val diff = Math.abs(sumKO - sumPiS) / (sumKO + sumPiS).toDouble
    (w.nazwa, diff)
  }

  val minDiffWojewództwo = wynikiPerWojewództwo.minBy(_._2)

  println(s"Województwo z minimalną różnicą procentową głosów: ${minDiffWojewództwo._1}, różnica: ${minDiffWojewództwo._2 * 100}%")
}
